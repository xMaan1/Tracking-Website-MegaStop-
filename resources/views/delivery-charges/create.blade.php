            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('delivery-charges.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="min_quantity" id="min_quantity" value="{{ old('min_quantity', 1) }}" min="1" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('min_quantity') border-red-500 @enderror" required>
                                @error('min_quantity')
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="max_quantity" id="max_quantity" value="{{ old('max_quantity') }}" min="1" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('max_quantity') border-red-500 @enderror">
                                @error('max_quantity')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="charge" id="charge" value="{{ old('charge') }}" step="0.01" min="0" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('charge') border-red-500 @enderror" required>
                                @error('charge')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="flex items-center">
                                    <input type="checkbox" name="is_multiplier" id="is_multiplier" value="1" {{ old('is_multiplier') ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div class="flex justify-end space-x-3">
                        ...
                        <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Create Rule</button>
                    </div>
                </form>
            </div>
        </div>
